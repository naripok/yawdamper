...
bool MPU6050::getTempEnabled(void)
{
    return !readRegisterBit(MPU6050_REG_PWR_MGMT_1, 3);
}

void MPU6050::setTempEnabled(bool state)
{
    writeRegisterBit(MPU6050_REG_PWR_MGMT_1, 3, !state);
}
...
SignedVector MPU6050::readRawAccel(void)
{
	int res = 0;
    ...
    Wire.beginTransmission(mpuAddress);
    res = Wire.requestFrom(mpuAddress, 6);
...
Vector MPU6050::readSignedAccel(void)
{
    readRawAccel();
